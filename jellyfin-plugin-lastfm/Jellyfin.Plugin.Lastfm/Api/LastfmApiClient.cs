...
public class LastfmApiClient
{
    ...
    public LastfmApiClient(IHttpClientFactory httpClientFactory, PluginConfiguration config, ILogger logger)
    {
        ...
        _httpClient.DefaultRequestHeaders.Add("User-Agent", "Jellyfin-Lastfm/10.9.0");
    ...
    public async Task UpdateNowPlayingAsync(string artist, string title, string? album = null)
    {
    ...
    public async Task ScrobbleAsync(string artist, string title, string? album = null, long timestamp = 0)
    {
        if (string.IsNullOrEmpty(_sessionKey))
        {
            return;
